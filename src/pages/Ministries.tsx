...
const MinistryRow: React.FC<{ ministry: MinistryResponseDto; index: number }> = ({ ministry, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-3/5 group overflow-hidden">
        <div className="relative aspect-video overflow-hidden bg-gray-100 rounded-sm shadow-2xl">
          {ministry.coverImageUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-fuchsia-100 to-purple-100 flex items-center justify-center">
          ...
          <div className="flex items-center gap-3 mb-8">
            {ministry.leaderImageUrl && (
              <img
                ...
                className="w-10 h-10 rounded-full object-cover border-2 border-fuchsia-200"
              />
            )}
            <div>
  ...
  return (
    <div className="bg-white relative animate-in fade-in duration-700">
      ...
      <section className="bg-[#0a0a0a] pt-32 pb-20 px-6">
        <div ref={rHero} style={fadeStyle} className="max-w-7xl mx-auto border-l border-white/20 pl-8">
          ...
          <p className="text-gray-400 max-w-2xl text-lg font-light leading-relaxed text-justify">
      ...
      {error && !isLoading && (
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">⚠️ {error}</div>
        </div>
      )}
      ...
      {!isLoading && !error && ministries.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <p className="text-slate-500 font-serif italic text-lg">No ministries available yet. Check back soon.</p>
        </div>
      )}
        ...
        <section className="max-w-7xl mx-auto px-6 py-24">
          <div className="flex flex-col gap-32">
            {ministries.map((ministry, index) => (
              <MinistryRow key={ministry.id} ministry={ministry} index={index} />
            ))}
          </div>
        </section>
      )}
