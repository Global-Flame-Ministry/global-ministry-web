...
import { Bell, Search, Calendar, Tag, ArrowRight, X, AlertCircle, ArrowLeft } from 'lucide-react';
...
const YouthAnnouncements: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      ...
      setError(null);
      try {
        const response = await announcementApi.getYouthAll({
          pageNumber,
          pageSize,
  ...
  return (
    <div className="bg-[#fafafa] min-h-screen font-sans selection:bg-fuchsia-100">
      ...
      <section className="bg-[#0a0a0a] pt-32 pb-20 relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_-20%,#4a044e,transparent)]
          opacity-40" />
        <div ref={rHero} style={fadeStyle}
          ...
          >
            <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-1
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full
              ...
              <span className="text-fuchsia-400 uppercase tracking-widest
                text-[10px] font-bold">
                Youth Updates
              </span>
            </div>
            ...
            <div className="max-w-2xl mx-auto relative group">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2
                text-white/30 w-5 h-5 group-focus-within:text-fuchsia-400
                transition-colors" />
              <input
                type="text"
                ...
                className="w-full bg-white/5 backdrop-blur-md border
                  ...
                  pl-14 pr-12 py-4 rounded-2xl outline-none
                  focus:ring-2 focus:ring-fuchsia-500/50
                  focus:border-fuchsia-500/50 transition-all shadow-2xl"
              />
              {searchQuery && (
                <button
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-8 relative z-20 pb-20">
        ...
        <div
          ref={rFilters}
          style={fadeStyle}
          className="flex flex-wrap items-center gap-3 mb-12 bg-white p-2
            rounded-2xl shadow-sm border border-slate-100 overflow-x-auto"
        >
          <button
        ...
        <div ref={rGrid} style={fadeStyle}>
          {error ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="animate-pulse border border-slate-100
              ...
              {announcements.map((item, idx) => (
                <article
                  ...
                  className="group bg-white border border-slate-100 rounded-2xl
                    p-8 hover:border-fuchsia-300 cursor-pointer flex flex-col
                    h-full"
                  style={{
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    <span className="px-3 py-1 bg-fuchsia-50 text-fuchsia-600
                  ...
                  <h3 className="text-2xl font-serif font-bold text-slate-900
                    mb-4 group-hover:text-fuchsia-600 transition-colors
                    leading-snug">
                    {item.title}
                  </h3>
                  <p className="text-slate-500 text-sm leading-relaxed
                    line-clamp-3 mb-8 flex-grow">
                    {item.content}
                  </p>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 mt-20">
            <button
              ...
              className="p-3 rounded-xl border border-slate-200 hover:bg-white
                disabled:opacity-30 transition-all"
            >
              <ArrowRight className="w-5 h-5 rotate-180" />
            </button>
            <span className="text-sm font-bold text-slate-400 mx-4">
              Page <span className="text-slate-900">{pageNumber}</span>{' '}
              of {totalPages}
            </span>
            <button
      ...
      {selectedAnnouncement && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-white w-full max-w-2xl rounded-[2rem]
            shadow-2xl overflow-hidden">
            <button
              onClick={() => setSelectedAnnouncement(null)}
              className="absolute top-6 right-6 p-2 rounded-full bg-slate-50
            ...
            <div className="p-10 pt-16">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-xs font-black text-fuchsia-600 uppercase
                  tracking-widest">
              ...
              <button
                onClick={() => setSelectedAnnouncement(null)}
                className="mt-12 w-full py-4 bg-slate-900 text-white
                  rounded-2xl font-bold uppercase tracking-widest
                  hover:bg-fuchsia-600 hover:shadow-xl transition-all"
              >
                Close
              </button>
            </div>
...
export default YouthAnnouncements;