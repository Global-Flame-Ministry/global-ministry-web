    ...
    <div className={`min-h-screen font-sans ${t.bg}`}>
      ...
      <div className={`px-8 py-4 border-b ${t.border} flex flex-wrap gap-3`}>
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" placeholder="Search by title, speaker, series..." value={search}
            ...
            {sermons.map(sermon => (
              <div key={sermon.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${t.row}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                      sermon.isPublished ? 'bg-emerald-500/20 text-emerald-600' : 'bg-amber-500/20 text-amber-600'
                    }`}>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    {sermon.videoUrl && <span className={`flex items-center gap-1 text-[10px] font-bold ${t.mutedtext}`}><Play className="w-3 h-3" /> Video</span>}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => openEdit(sermon)} className={`p-2 rounded-lg transition-colors ${t.btnGhost}`}>
                    <Pencil className={`w-4 h-4 ${t.subtext}`} />
                  </button>
                  <button onClick={() => setDeleteTarget(sermon)} className={`p-2 rounded-lg transition-colors ${t.btnGhost} hover:bg-red-500/20`}>
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={form.speaker} onChange={e => setForm(p => ({ ...p, speaker: e.target.value }))}
                ...
                <div>
                  ...
                  <input type="text" value={form.series} onChange={e => setForm(p => ({ ...p, series: e.target.value }))}
              ...
              <div className="grid grid-cols-2 gap-3 items-end">
                <div>
                  ...
                  <input type="date" value={form.sermonDate} onChange={e => setForm(p => ({ ...p, sermonDate: e.target.value }))}
                    className={`w-full px-4 py-3 border rounded-xl text-sm outline-none ${t.modalInput}`} />
                </div>
                <div className="pb-1">
                  <div className="flex items-center gap-3 cursor-pointer"
                    ...
                    <span className={`text-sm ${t.subtext}`}>{form.isPublished ? 'Published' : 'Draft'}</span>
                  </div>
                </div>
              </div>
              ...
              {form.audioUrl && (
                <div className={`p-4 rounded-xl border ${t.border} space-y-3`}>
                  <p className={`text-xs font-bold uppercase tracking-widest ${t.subtext}`}>Audio Preview</p>
                  <audio controls className="w-full">
                    <source src={form.audioUrl} />
                  </audio>
                  <a href={form.audioUrl} download
