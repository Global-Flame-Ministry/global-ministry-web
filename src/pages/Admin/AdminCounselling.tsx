...
import {
  ...
  ChevronLeft, ChevronRight, UserCheck, Clock, ArrowLeft
} from 'lucide-react';
...
const AdminCounselling = () => {
  ...
  // On mobile we show either the list OR the detail — not both side-by-side
  const [selected, setSelected]         = useState<CounsellingResponseDto | null>(null);
  const pageSize = 10;

  const [showAssign, setShowAssign]         = useState(false);
  const [assignForm, setAssignForm]         = useState({ assignedTo: '', assignedToEmail: '' });
  ...
  const DetailPanel = () => {
    ...
    return (
      <div className="flex flex-col h-full overflow-hidden">
        {/* Panel Header */}
        <div className={`px-4 sm:px-8 py-4 sm:py-5 border-b ${t.border}
          flex items-center justify-between gap-3 shrink-0`}>
          <div className="flex items-center gap-3 min-w-0">
            {/* Back button — shown on mobile where it replaces the list */}
            <button
              onClick={() => setSelected(null)}
              className={`lg:hidden p-2 rounded-lg transition-colors ${t.btnGhost} shrink-0`}
            >
              <ArrowLeft className={`w-4 h-4 ${t.subtext}`} />
            </button>
            <div className="min-w-0">
              <h2 className="font-bold text-base sm:text-lg truncate">{selected.fullName}</h2>
          ...
          <button
            onClick={() => setSelected(null)}
            className={`hidden lg:flex p-2 rounded-lg transition-colors ${t.btnGhost} shrink-0`}
          >
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-4 sm:py-6 space-y-4">
          ...
          <div className={`rounded-xl p-4 sm:p-5 border ${t.card}`}>
            <p className={`text-xs uppercase tracking-widest mb-3 ${t.subtext}`}>
              Contact Information
            </p>
          ...
          {/* Topic + Message */}
          <div className={`rounded-xl p-4 sm:p-5 border ${t.card}`}>
            <p className={`text-xs uppercase tracking-widest mb-1 ${t.subtext}`}>
              Topic
            </p>
            <p className="font-bold mb-4">{selected.topic}</p>
            <p className={`text-xs uppercase tracking-widest mb-2 ${t.subtext}`}>
              Message
            </p>
        ...
        {/* Footer Actions */}
        <div className={`px-4 sm:px-8 py-4 sm:py-5 border-t ${t.border} shrink-0`}>
          <button
            onClick={() => {
              setAssignForm({
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2
              px-5 py-3 bg-fuchsia-600 hover:bg-fuchsia-500 text-white rounded-xl
              text-sm font-bold uppercase tracking-widest transition-colors"
          >
            <UserCheck className="w-4 h-4" />
            {selected.assignedTo ? 'Reassign Counsellor' : 'Assign Counsellor'}
          </button>
        </div>
      </div>
    );
    ...
    <div className={`min-h-screen font-sans ${t.bg}`}>
      ...
      <div className={`px-4 sm:px-8 pt-6 sm:pt-8 pb-4 sm:pb-6 border-b ${t.border}
        ...
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold">Counselling Requests</h1>
        </div>
      ...
      {/* Filters — hidden when viewing detail on mobile */}
      {!selected && (
      ...
      {/* ── BODY ─────────────────────────────────────────────── */}
      {/* 
        Mobile:  show list OR detail (never both simultaneously)
        Desktop: show list + detail side-by-side (lg:flex)
      */}
      <div className="lg:flex lg:h-[calc(100vh-180px)]">

        {/* LIST — hidden on mobile when a request is selected */}
        <div className={`
          flex flex-col
          ${selected ? 'hidden lg:flex lg:w-2/5' : 'flex w-full'}
          lg:border-r ${t.border}
        `}>
          <div className="flex-1 overflow-y-auto p-3 sm:p-4">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className={`text-sm ${t.subtext}`}>No counselling requests found</p>
              </div>
            ) : (
              <div className="space-y-2">
                {requests.map(r => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span className={`text-[10px] font-bold uppercase px-2 py-0.5
                        rounded-full flex-shrink-0 ${statusBadgeClass(r.status)}`}>
                        {r.status}
                      </span>
                    </div>
                    <p className={`text-xs truncate mb-1.5 ${t.subtext}`}>{r.topic}</p>
        ...
        {/* DETAIL — full screen on mobile when selected, side panel on desktop */}
        {selected && (
          <div className={`
        ...
        {!selected && (
          <div className="hidden lg:flex flex-1 items-center justify-center text-center px-8">
            <div>
              <Heart className={`w-10 h-10 mb-4 mx-auto ${t.mutedtext}`} />
              <p className={`text-sm ${t.subtext}`}>Select a request to view details</p>
            </div>
          </div>
        )}
      </div>
      ...
      {showAssign && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          />
          <div className={`relative rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md
            border p-6 sm:p-8 ${t.modal}`}>
            <button
              onClick={() => setShowAssign(false)}
              className={`absolute top-4 right-4 p-1.5 rounded-lg transition-colors ${t.btnGhost}`}
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 border rounded-xl text-sm outline-none ${t.input}`}
                />
              </div>
              <div>
                <label className={`text-xs font-bold uppercase tracking-widest
                ...
                <input
                  ...
                  onChange={e => setAssignForm(p => ({ ...p, assignedToEmail: e.target.value }))}
                  placeholder="pastor@globalflame.org"
                  className={`w-full px-4 py-3 border rounded-xl text-sm outline-none ${t.input}`}
                />
              </div>
              <p className={`text-xs ${t.subtext}`}>
                The counsellor will receive an email with the full details of this request.
              </p>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => setShowAssign(false)}
                className={`flex-1 py-3 rounded-xl text-sm transition-colors ${t.btnGhost}`}
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-500
